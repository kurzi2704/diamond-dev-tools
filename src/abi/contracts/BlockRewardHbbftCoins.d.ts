...
export interface BlockRewardHbbftCoins extends BaseContract {
  constructor(
    jsonInterface: any[],
    address?: string,
    options?: ContractOptions
  ): BlockRewardHbbftCoins;
  clone(): BlockRewardHbbftCoins;
  methods: {
    ...
    /**
     * Called by the `StakingHbbft.claimReward` function to transfer native coins from the balance of the `BlockRewardHbbft` contract to the specified address as a reward.
    ...
    /**
     * Returns the reward amount in native coins for some delegator with the specified stake amount placed into the specified pool before the specified staking epoch. Used by the `StakingHbbft.claimReward` function.
     ...
     * @returns `uint256 nativeReward` - the reward amount in native coins.
     */
    getDelegatorReward(
    ...
    /**
     * Returns the reward amount in native coins for the specified validator and for the specified staking epoch. Used by the `StakingHbbft.claimReward` function.
     * @param _poolMiningAddress The pool mining address.
     * @param _stakingEpoch The serial number of staking epoch.
     * @returns `uint256 nativeReward` - the reward amount in native coins.
     */
